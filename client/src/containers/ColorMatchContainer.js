...
class ColorMatchContainer extends React.Component {
    ...
    componentDidMount() {
        let type = types[Math.floor(Math.random() * len(types))];
        let correct_rand = Math.floor(Math.random() * len(colors));
        ...
        let incorrect_color = colors[Math.floor(Math.random()*len(colors))];
        let incorrect_word = words[Math.floor(Math.random() * len(words))];
        ...
        while (type == "word" && incorrect_word == correct_word) {
            incorrect_word = words[Math.floor(Math.random() * len(words))];
        }

        while (type == "color" && incorrect_color == correct_color) {
            incorrect_color = colors[Math.floor(Math.random()*len(colors))];
        }
